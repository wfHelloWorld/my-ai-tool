      ...
      <el-splitter-panel :min="300">
        <div class="h-full w-full">
          <el-scrollbar style="height: 100%">
            <div class="p-4">
              <MarkdownViewer :source="wan22Kf2vMd" />
            </div>
          </el-scrollbar>
        </div>
      </el-splitter-panel>
              ...
              <div class="flex-1 overflow-y-auto px-[5%]">
                ...
                <div class="p-4 border rounded bg-gray-50 text-gray-800 space-y-3">
                  ...
                  <div class="grid grid-cols-2 gap-4 mt-4">
                    <!-- 首帧图片 (必填) -->
                    <div>
                      <div class="font-semibold mb-2">首帧图片 (必填)</div>
                      <div class="flex items-center gap-3">
                        <div v-if="firstFrameItem" class="relative group">
                          <img :src="firstFrameItem.preview" class="w-32 h-32 object-contain rounded border bg-gray-200" />
                          <button class="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity"
                            @click="removeFirstFrame">×</button>
                        </div>
                        <div v-else class="w-32 h-32 border-2 border-dashed border-gray-300 rounded flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 hover:text-blue-500 text-gray-400" @click="triggerFirstFrameSelect">
                          <el-icon class="text-2xl mb-1"><Icon icon="mdi:image-plus" /></el-icon>
                          <span class="text-xs">选择首帧</span>
                        </div>
                      </div>
                      <input ref="firstFrameInputRef" type="file" accept="image/*" class="hidden" @change="handleFirstFrameSelect" />
                    </div>

                    <!-- 尾帧图片 (选填) -->
                    <div>
                      <div class="font-semibold mb-2">尾帧图片 (选填)</div>
                      <div class="flex items-center gap-3">
                        <div v-if="lastFrameItem" class="relative group">
                          <img :src="lastFrameItem.preview" class="w-32 h-32 object-contain rounded border bg-gray-200" />
                          <button class="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity"
                            @click="removeLastFrame">×</button>
                        </div>
                        <div v-else class="w-32 h-32 border-2 border-dashed border-gray-300 rounded flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 hover:text-blue-500 text-gray-400" @click="triggerLastFrameSelect">
                          <el-icon class="text-2xl mb-1"><Icon icon="mdi:image-plus" /></el-icon>
                          <span class="text-xs">选择尾帧</span>
                        </div>
                      </div>
                      <input ref="lastFrameInputRef" type="file" accept="image/*" class="hidden" @change="handleLastFrameSelect" />
                    </div>
                  </div>
                </div>
...
import wan22Kf2vMd from "../../common/md/video/wan2.2-kf2v-flash.md?raw";
import { useWanxiang22Kf2vStatusStore } from "../../stores/useWanxiang22Kf2vStatusStore";
import { useProvidersStore } from "../../stores/useProviderStore";

const wanStore = useWanxiang22Kf2vStatusStore();
...
// 首帧
const firstFrameItem = ref<ImageItem | null>(null);
const firstFrameInputRef = ref<HTMLInputElement | null>(null);
const triggerFirstFrameSelect = () => firstFrameInputRef.value?.click();
const handleFirstFrameSelect = async (e: Event) => {
  const input = e.target as HTMLInputElement;
  if (!input.files?.length) return;
  const file = input.files[0];
  try {
    const storedPath = await (window as any).electronAPI.ensureImageStored(file);
    firstFrameItem.value = {
      path: storedPath,
...
const removeFirstFrame = () => {
  if (firstFrameItem.value) URL.revokeObjectURL(firstFrameItem.value.preview);
  firstFrameItem.value = null;
};

// 尾帧
const lastFrameItem = ref<ImageItem | null>(null);
const lastFrameInputRef = ref<HTMLInputElement | null>(null);
const triggerLastFrameSelect = () => lastFrameInputRef.value?.click();
const handleLastFrameSelect = async (e: Event) => {
  const input = e.target as HTMLInputElement;
  if (!input.files?.length) return;
  const file = input.files[0];
  try {
...
const removeLastFrame = () => {
  if (lastFrameItem.value) URL.revokeObjectURL(lastFrameItem.value.preview);
  lastFrameItem.value = null;
};
...
onMounted(() => {
  unsubscribe = (window as any).electronAPI.onWan22Kf2vFlashProgress((info: any) => {
...
const createTask = async (prompt: string) => {
  if (!firstFrameItem.value) {
    alert("请先选择首帧图片");
    return;
  }

  const clientId = `${Date.now()}-${Math.random().toString(36).slice(2,7)}`;
  const taskName = prompt ? prompt.slice(0, 20) : "首尾帧视频任务";
  ...
  try {
    ...
    const provider = providersStore.items.find(p => p.name === "wan2.2-kf2v-flash");
    
    const payload = {
      prompt,
      negative_prompt: params.negative_prompt,
      firstFramePath: firstFrameItem.value.path,
      lastFramePath: lastFrameItem.value?.path,
    ...
    const res = await (window as any).electronAPI.startWan22Kf2vFlash(payload);
    // 将新结果添加到列表头部
    if (res && res.length > 0) {
